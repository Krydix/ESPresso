                              ...
                              size_t *capacity)
{
    ...
    *capacity = wanted;
    ...
    int received = httpd_req_recv(request, (char *)*buffer + *length, wanted - *length);
    if (received <= 0) {
        return ESP_FAIL;
    }
                                          ...
                                          size_t remaining)
{
    ...
    while (remaining > 0) {
        ...
        int received = httpd_req_recv(request, (char *)chunk, wanted);
        if (received <= 0) {
            err = ESP_FAIL;
            break;
        }
...
static esp_err_t proxy_handler(httpd_req_t *request)
{
    ...
    snprintf(upstream_url, sizeof(upstream_url), "http://%s:%u%s", target.address,
             target.port, target.resource_path);
    snprintf(upstream_printer_uri, sizeof(upstream_printer_uri), "ipp://%s:%u%s",
             target.address, target.port, target.resource_path);
    ...
    while (codec_result == IPP_CODEC_INCOMPLETE) {
        ...
        codec_result = ipp_codec_rewrite(prefix, prefix_length, upstream_printer_uri,
                                         upstream_authority, &rewritten_prefix,
                                         &rewritten_length, &attributes_length);
    }
    ...
    uint8_t client_major = rewritten_prefix[0];
    uint8_t client_minor = rewritten_prefix[1];
    uint16_t operation_id = (uint16_t)(((uint16_t)rewritten_prefix[2] << 8) |
                                       rewritten_prefix[3]);
    ...
    if (operation_id == 0x000b) {
        if ((size_t)request->content_len != attributes_length) {
            free(rewritten_prefix);
            free(prefix);
            httpd_resp_send_err(request, HTTPD_400_BAD_REQUEST,
                                "Get-Printer-Attributes cannot contain document data");
            return ESP_FAIL;
        }
        uint32_t request_id = ((uint32_t)rewritten_prefix[4] << 24) |
                              ((uint32_t)rewritten_prefix[5] << 16) |
                              ((uint32_t)rewritten_prefix[6] << 8) |
                              rewritten_prefix[7];
        uint8_t *capability_request = NULL;
        size_t capability_request_length = 0;
        codec_result = ipp_codec_build_get_printer_attributes(
            upstream_major, upstream_minor, request_id, upstream_printer_uri,
            upstream_major >= 2, &capability_request, &capability_request_length);
        free(rewritten_prefix);
        if (codec_result != IPP_CODEC_OK) {
            free(prefix);
